...
def ask_api(lcserve_host: str, url: str, file: _TemporaryFileWrapper, question: str, claude_key: str) -> str:
    ...
    if url.strip() != '':
        ...
        with open(file.name, 'rb') as f:
            r = requests.post(f'{lcserve_host}/ask_file', params={'input_data': json.dumps(_data)}, files={'file': f})
...
title = 'PDF Claude'
description = """PDF Claude allows you to chat with your PDF file using Universal Sentence Encoder and Claude. It gives hallucination-free responses compared to other tools as the embeddings are better than OpenAI. The returned response can even cite the page number in square brackets ([]) where the information is located, adding credibility to the responses and helping to locate pertinent information quickly."""

st.title(title)
st.markdown(description)

lcserve_host = st.text_input(label='Enter your API Host here', value='http://localhost:8080')
st.markdown(f'<p style="text-align:center">Get your Claude API key <a href="https://console.anthropic.com/account/api-keys">here</a></p>', unsafe_allow_html=True)
...
file = st.file_uploader(label='Upload your PDF/Research Paper/Book here', type=['pdf'])
